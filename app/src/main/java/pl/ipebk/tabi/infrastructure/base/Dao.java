...
public abstract class Dao<E extends Model> {
    ...
    public void delete(Long id) {
        int rowsAffected = db.delete(table.getName(), BaseColumns._ID + " = " + id);
        if (rowsAffected < 1) {
    ...
    public void deleteList(List<Long> ids) {
        ...
        int rowsAffected = db.delete(table.getName(),
    ...
    public int deleteAll() {
        int rowsAffected = db.delete(table.getName(), "1");
        Timber.d("Rows deleted: %d", rowsAffected);
        return rowsAffected;
    }
    ...
    public Observable<E> getByIdObservable(Long id) {
        ...
        return db.createQuery(table.getName(), sql, selectionArgs)
    ...
    public Observable<List<E>> getAllObservable() {
        String sql = SQLiteQueryBuilder.buildQueryString(false, table.getName(),
                table.getQualifiedColumns(), null, null, null, null, null);
        return db.createQuery(table.getName(), sql)
    ...
    public Observable<Cursor> getAllCursorObservable() {
        String sql = SQLiteQueryBuilder.buildQueryString(false, table.getName(),
                table.getQualifiedColumns(), null, null, null, null, null);
        return db.createQuery(table.getName(), sql).map(SqlBrite.Query::run);
    }
    ...
    public E update(E model) {
        ...
        if (id > 0) {
            ...
            String selection = BaseColumns._ID + " = ?";
            String[] selectionArgs = {String.valueOf(id)};
            int rowsAffected = db.update(table.getName(), values, selection, selectionArgs);
